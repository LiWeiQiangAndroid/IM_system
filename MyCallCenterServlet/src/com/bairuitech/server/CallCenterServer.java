...
import com.wandou.verify.DbHandler;
...
public class CallCenterServer implements AnyChatServerEvent{
	...
	public static void initSDK()  {
		...
		System.out.print(getCurrentTime() + "Welcome use AnyChat! ("
				+ anychat.GetSDKVersion() + ")\r\n");
//		Reader reader = new InputStreamReader(System.in);
//		char ch =0;
//		do {
//			if (ch != '\r') // 为什么要做判断不懂，\r是回车符
//				System.out.print(getCurrentTime()
//						+ "AnyChatCallCenter server(Java) still running, press 'q' to exit...\r\n");						
//			Thread.sleep(100);
//		} while ((ch = (char) reader.read()) != 'q');
//
//		anychat.Release();
//		System.out.print(getCurrentTime()
//				+ "AnyChatCallCenter server(Java) already exited...\r\n");
	}
	...
	public int OnAnyChatVerifyUserCallBack(String szUserName, String szPassword, AnyChatVerifyUserOutParam outParam) {
		...
		boolean b = db.checkUser("select * from user where user_name=? and user_psd=?",new String[]{szUserName,szPassword});
		if (b) {
			verifyNum = 0;
			...
			System.out.print(getCurrentTime() + "OnVerifyUserCallBack: userid:"
					+ iUserIdSeed + " username: " + szUserName + " userpwd:"
					+ szPassword + "\r\n");
			iUserIdSeed += 1;

		}else {
			verifyNum = 100;//验证用户名密码出错码，在客户端的LoginMessage对应
		}
		return  verifyNum ;
	}
